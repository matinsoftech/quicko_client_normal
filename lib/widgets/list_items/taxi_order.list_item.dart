    ...
    return VStack(
      [
        ...
        HStack(
          [
            //price
            "${AppStrings.currencySymbol}${order.total}".text.semiBold.xl.make().expand(),
            //status
            "${order.Taxistatus.allWordsCapitilize() ?? ''}".text.color(AppColor.getStausColor(order.status)).make(),
          ],
        ).py8().px20(),
      ],
    )
